                ...
                <tbody class="bg-white">
                    @forelse ($transaksi as $transaksi)
                        <tr class="border-b hover:bg-slate-50 transition-all duration-300">
                            ...
                            <td class="px-6 py-4">{{ $transaksi->pelanggan->name_222405 ?? 'Nama Tidak Ditemukan' }}</td>
                            <td class="px-6 py-4">{{ $transaksi->produk->nama_222405 ?? 'Produk Tidak Ditemukan' }}</td>
                            <td class="px-6 py-4">{{ $transaksi->jumlah_222405 }}</td>
                            <td class="px-6 py-4">Rp {{ number_format($transaksi->harga_total_222405, 0, ',', '.') }}</td>
                            <td class="px-6 py-4">
                                <select
                                    class="status-dropdown text-green-800 hover:bg-indigo-100 focus:outline-none rounded-lg shadow-sm"
                                    data-id="{{ $transaksi->id_transaksi_222405 }}" onchange="updateStatus(this)">
                                    <option value="pending" {{ $transaksi->status_222405 == 'pending' ? 'selected' : '' }}>
                                        Pending
                                    </option>
                                    <option value="dikemas" {{ $transaksi->status_222405 == 'dikemas' ? 'selected' : '' }}>
                                        Dikemas
                                    </option>
                                    <option value="dikirim" {{ $transaksi->status_222405 == 'dikirim' ? 'selected' : '' }}>
                                        Dikirim
                                    </option>
                                    <option value="selesai" {{ $transaksi->status_222405 == 'selesai' ? 'selected' : '' }}>
                                        Selesai
                                    </option>
                                </select>
                            </td>
                            <td class="px-6 py-4">
                                {{ \Carbon\Carbon::parse($transaksi->tanggal_transaksi_222405)->format('d-m-Y') }}</td>

                            <td class="px-6 py-4 flex gap-1">
                                <button onclick="showBuktiTF('{{ asset('storage/' . $transaksi->bukti_tf_222405) }}')"
                                ...
                                <form action="{{ route('admin.transaksi.destroy', $transaksi->id_transaksi_222405) }}"
                                    method="POST"
